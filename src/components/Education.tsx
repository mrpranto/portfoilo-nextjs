...
const colorMap: Record<string, { icon: string; border: string; tag: string }> = {
  cyan:   { icon: "text-cyan-600 dark:text-cyan-400 bg-cyan-500/10",     border: "border-l-cyan-500/50",   tag: "bg-cyan-500/10 text-cyan-700 dark:text-cyan-300 border-cyan-500/20" },
  purple: { icon: "text-purple-600 dark:text-purple-400 bg-purple-500/10", border: "border-l-purple-500/50", tag: "bg-purple-500/10 text-purple-700 dark:text-purple-300 border-purple-500/20" },
};
  ...
  return (
    <section id="education" className="py-24 relative">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-16">
          <p className="text-cyan-600 dark:text-cyan-400 font-mono text-sm mb-3">{"// academic background"}</p>
        ...
        <div className="grid md:grid-cols-2 gap-6">
          {education.map((edu, i) => {
            const cfg = colorMap[edu.color];
            return (
              <div key={i} className={`glass-card glass-card-hover border-l-2 ${cfg.border} p-6`}>
                <div className="flex items-start gap-4 mb-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm mt-1" style={{ color: "var(--text-2)" }}>{edu.institution}</p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 text-sm mb-4" style={{ color: "var(--text-3)" }}>
                  <Calendar size={13} />
                  <span>{edu.period}</span>
                </div>
                <p className="text-sm leading-relaxed mb-4" style={{ color: "var(--text-2)" }}>{edu.description}</p>
                <div className="flex flex-wrap gap-2">
                  {edu.highlights.map((h) => (
                    <span key={h} className={`text-xs font-medium px-2.5 py-1 rounded-full border ${cfg.tag}`}>{h}</span>
                  ))}
                </div>
              </div>
            );
        ...
        <div className="mt-8 glass-card p-6">
          <p className="text-sm text-center" style={{ color: "var(--text-2)" }}>
            Continuous learner — staying current with emerging technologies through hands-on projects,
            open-source contributions, and production SaaS development.
          </p>
        </div>
      </div>
    </section>
